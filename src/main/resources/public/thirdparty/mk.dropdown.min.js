angular.module("mk.dropdown.tpl", []).run(["$templateCache", function (a) {
    a.put("templates/mk.dropdown.tpl.html", '' +
        '<div class="input-group" style="padding-right: 33px"> ' +
        '<div id="buttonDiv" class="{{buttonClass}}"> ' +
        '<input ng-change="toggleDropdown()" ng-keydown="moveFocus($event)" ng-focus="inputFocus()" ng-blur="setItem()" type="text" class="form-control" ' +
        'placeholder="{{placeholder}}" ng-model="itemValue" aria-label="..."> ' +
        '<ul id="dropdownUl" class="dropdown-menu" style="{{displayType}}"> ' +
        '<li ng-repeat="item in items | filter:filterOnLocation" ng-keydown="onKeyDown($event, item)" ng-mousedown="onMouseDown(item)">' +
        '<a href="" ng-blur="setItem()">{{itemLabel?item[itemLabel]:item}}</a>' +
        '</li> ' +
        '</ul> ' +
        '<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" ng-focus="buttonFocus()" ng-click="buttonClick()" ng-blur="setItem()">' +
        '<span class="caret"></span>' +
        '</button> ' +
        '</div> ' +
        '</div>')
}]), angular.module("mk.dropdown", ["mk.dropdown.tpl"]).directive("mkDropdown", function () {
        return {
            templateUrl: "templates/mk.dropdown.tpl.html",
            transclude: 'true',
            scope: {
                itemValue: "=?",
                items: "=",
                itemLabel: "@?",
                itemId: "@?",
                item: "=?",
                placeholder: "@?"
            },
            replace: true,
            controller: ["$scope", "$q", function ($scope, $timeout, $q) {
                $scope.buttonClass = "input-group-btn";
                $scope.skipInput = false;
                $scope.displayType = "display:none";

                if (!$scope.itemId) {
                    $scope.itemId = "id";
                }
                if($scope.item){
                    $scope.itemValue = $scope.item[$scope.itemLabel];
                }
                $scope.onKeyDown = function (event, item) {
                    if (event.which === 13) {
                        $scope.itemValue = angular.copy($scope.itemLabel?item[$scope.itemLabel]:item);
                    }
                    $scope.displayType = "display:none";
                };

                $scope.onMouseDown = function (item) {
                    $scope.itemValue = angular.copy($scope.itemLabel?item[$scope.itemLabel]:item);
                    $scope.displayType = "display:none";
                };

                $scope.toggleDropdown = function () {
                    $scope.buttonClass = "input-group-btn open";
                    $scope.displayType = "display:inline";
                };

                $scope.inputFocus = function(){
                    $scope.skipInput = false;
                    //$scope.buttonClass = "input-group-btn";
                }

                $scope.buttonClick = function(){
                    $scope.skipInput = true;
                    if($scope.displayType == "display:inline") {
                        $scope.displayType = "display:none";
                    }else{
                        $scope.displayType = "display:inline";
                    }
            }

                $scope.setItem = function () {
                    $scope.item = null;
                    if ($scope.itemValue) {
                        if ($scope.itemLabel) {
                            $scope.items.forEach(function (it, index) {
                                if (it[$scope.itemLabel].toLowerCase() == $scope.itemValue.toLowerCase()) {
                                    $scope.item = it;
                                }
                            });
                        } else {
                            if ($scope.items.indexOf($scope.itemValue) > -1) {
                                $scope.item = angular.copy($scope.itemValue);
                            }
                        }
                    }
                    $scope.displayType = "display:none";
                }

                $scope.moveFocus = function (event) {
                    $scope.toggleDropdown();
                    if (event.which === 40) {
                        var element = angular.element(document.querySelector(' #dropdownUl'));
                        setTimeout(function () {
                            element.find("a:first").focus();
                        }, 100);
                    }
                };
                $scope.filterOnLocation = function (item) {
                    if ($scope.itemValue && !$scope.skipInput) {
                        var match = null;
                        if($scope.itemLabel) {
                            match = (item[$scope.itemLabel]).toLowerCase().indexOf($scope.itemValue.toLowerCase()) >= 0;
                        }else{
                            match = item.toLowerCase().indexOf($scope.itemValue.toLowerCase()) >= 0;
                        }
                        return match;
                    } else {
                        return true;
                    }
                };
            }
            ]
        }
    }
);